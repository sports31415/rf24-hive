...
def try_read_data(channel=0):
    if radio.available():
        while radio.available():
            ...
            try:
                temperatures = receive_payload.decode('utf-8').rstrip(' \t\r\n\0\;')  # remove null character & ;
                temperatures = str(temperatures).split(';')
                # dict_temps = dict(enumerate(temperatures))
                # print(dict_temps)

                load_datetime = dt.datetime.now()
                for id in range(len(temperatures)):
                    insert_temperature(session, table_class, id, temperatures[id], load_datetime)
                db_commit(session)

                # insert_temperature(session, table_class, temp_json['id'], temp_json['temp'], load_datetime)
            except:
                print('Got payload size={} value="{}"'.format(length, receive_payload.decode('utf-8')))
...
session, table_class = set_connection()
...
while 1:
    if inp_role == '1':   # ping out
        ...
        if timeout:
            ...
            # json.loads(receive_payload.decode('utf-8'))
            # print(json.loads)
